...
static uint16_t checksum(uint8_t *buf, int16_t len,uint8_t type){
        // type 0=ip 
        //      1=udp
        //      2=tcp
        uint32_t sum = 0;
...
void make_udp_reply_from_request(uint8_t *buf,uint8_t *data, uint16_t datalen, uint16_t port)
{
		...
		len = IP_HEADER_LEN + UDP_HEADER_LEN + datalen;
		...
		len = UDP_HEADER_LEN + datalen;
        buf[UDP_LEN_H_P]=len >> 8;
        buf[UDP_LEN_L_P]=len & 0xFF;
        // zero the checksum
